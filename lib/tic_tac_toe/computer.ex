defmodule TicTacToe.Computer do
  ...
  #   Map.new
  #   |> Map.put(1.0, my_char)
  #   |> Map.put(-1.0, their_char)
  #   |> Utils.wrap_pre(:ok)
  # end

  def handle_call(:state, _from, state), do: {:ok, state, state}

  # def handle_call({:next_move, {board, valids, win_state}}, _from, chars = %{1.0 => my_char}) do
  def handle_call({:next_move, {board, valids, win_state}}, _from, chars = {my_char, their_char}) do
    ...
    best_move =
      ...
      |> case do
        {child_states, _, _} ->
          child_states
          |> reduce_child_states(-0.5, {their_char, my_char})
          |> IO.inspect
          |> elem(1)
          |> IO.inspect
  ...
  def reduce_child_states(child_states, mult, {char, next_char}) do
    child_states
    |> Enum.reduce(nil, fn({before_move, after_move, win_state, root_move}, last_score)->
      rem_moves =
        before_move
      ...
      |> case do
        {child_states, _, _} ->
          child_states
          |> reduce_child_states(mult * -0.5, {next_char, char})

         root_move_score ->
           {root_move_score, root_move}
      end
      |> IO.inspect
      |> max(last_score)
    end)
